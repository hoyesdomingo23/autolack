...
import datosProductosPoliester from "./datosProductosPoliester"; // Importa los datos correctamente
import "./ProductoPoliester.css";
import bannerProductos from "../../images/Poliester1920x920.jpg";
import bannerProductosM from "../../images/Poliester1000x500.jpg";

const Poliester = () => {
  const [productos, setProductos] = useState(datosProductosPoliester);
  ...
  const manejarBusqueda = (e) => {
    ...
    const filtrados = datosProductosPoliester.filter((producto) =>
  ...
  const manejarOrden = (tipoOrden) => {
    if (tipoOrden === "az") {
      ...
      setProductos(datosProductosPoliester);
    }
  ...
  return (
    <>
      ...
      <div className="contenedor-h2-productos">
        <h2>Descubre todos nuestros productos con Poliester!</h2>
      </div>
        ...
        <div className="grid">
          {productos.length > 0 ? (
            productos.map((producto, index) => (
              <Link
                to={`/ProductosPoliester/${encodeURIComponent(producto.text)}`}
                key={index}
                className="producto"
              >
