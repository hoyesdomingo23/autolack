...
const Monocomponentes = () => {
  ...
  const manejarBusqueda = (e) => {
    ...
    const filtrados = datosProductoMonocomponentes.filter((producto) =>
  ...
  const manejarOrden = (tipoOrden) => {
    if (tipoOrden === "az") {
      ...
      setProductos(datosProductoMonocomponentes);
    }
        ...
        <div className="grid">
          {productos.length > 0 ? (
            productos.map((producto, index) => (
              <Link
                to={`/ProductoMonocomponentes/${encodeURIComponent(
                  producto.text
                )}`}
                key={index}
                className="producto"
...
export default Monocomponentes;
