...
const Industria = () => {
  ...
  const manejarBusqueda = (e) => {
    ...
    const filtrados = datosProductoIndustria.filter((producto) =>
  ...
  const manejarOrden = (tipoOrden) => {
    if (tipoOrden === "az") {
      ...
      setProductos(datosProductoIndustria);
    }
  ...
  return (
    <>
      ...
      <div className="contenedor-h2-productos">
        <h2>Descubre todos nuestros productos para industria!</h2>
      </div>
        ...
        <div className="grid">
          {productos.length > 0 ? (
            productos.map((producto, index) => (
              <Link
                to={`/ProductoIndustria/${encodeURIComponent(producto.text)}`}
                key={index}
                className="producto"
              >
